...
def image_similarity(fp1, fp2):
    ...
    new_resize_image = image.resize((w, h))
    ...
    spectrum1 = 20 * np.log(cv2.magnitude(dtf_shift1[:, :, 0], dft_shift1[:, :, 1]))
    spectrum2 = 20 * np.log(cv2.magnitude(dtf_shift2[:, :, 0], dft_shift2[:, :, 1]))
...
test = image_similarity('User_Drawings\panda.png', 'User_Drawings\panda1.png')
print(test)
